...
## 進行中
- [ ] R-0144: Dodaikun v1受入テスト自動化のためのBeaver_beta向け機能追加。仕様: `docs/spec/R-0144_beaver_beta_uat_support.md`。急ぎではない（frontpc側が先に開発環境で通しシナリオを作る段階）。B-1（スナップショット保存・復元API、本番誤爆防止の多重歯止め必須）・B-2（`/sync/status`の認証ゲート免除追加）・B-4（`GET /invoices/sync`・`GET /payments/sync`新設）が実装対象。B-3は既存API仕様の返信のみ
