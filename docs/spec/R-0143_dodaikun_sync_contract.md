...
| A-B-03 | `GET/POST /vouchers/sync` に `lines[]` | — | `test_vouchers_sync_lines.php`: 5 列が往復で一致、`lines_mode` の replace/merge | done（9fa585b、Beaver_betaで実機確認済み。`price`は`line_total`にマッピング — Access側の期待値と一致するかDodaikun側で要確認） |
