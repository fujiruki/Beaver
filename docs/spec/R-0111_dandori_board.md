...
### 追加フィードバック（2026-08-24、本番feedback id=27）

原文: 「この画面の時、例えば「オム」と書いてるチャート部分は、「オムロ扉修繕」という案件名なんです。いまはチャートの長さで切れてるけど、文字が右にはみ出して描画されていいです。ただ、白文字では読めないよね？どんなデザインにするのがいいんだろう？？」

- **F5: バーに収まらないラベルは外側に出す**: ラベルの必要幅がバー幅を超える場合、ラベルをバーの右外側に**地の文字色（--ink）**で描画する（白文字のはみ出しは背景で読めないため）。バー内に収まる場合は従来どおり白文字でバー内に表示。判定はバー幅とラベル文字数×フォント幅の概算でよい（厳密なmeasureTextは不要）。外側ラベルが納期線や隣接要素に重なるのは許容（ドラッグ操作の妨げにならないよう pointer-events: none）
  - 適用範囲は横スクロールモードのみ（実装判断2026-08-24: 折り返しモードはレーン幅が%・週単位で変動し概算判定が複雑化するため対象外。必要になったら別要望で）
