...
- **タイムゾーン変換漏れ修正（完了・2026-09-08）**: A-B-11後の手順7再実行で、得意先beaver_newerが523件超も誤って積まれる問題が発覚。DodaikunがAccess側実データで裏取りし（得意先№812・№755の具体例、9時間のズレ＝UTC/JST変換の有無の差）、`GET /customers/sync`は`utcToJst()`変換済みだが`customerAccessLink()`・POST customer upsert（update/insert両分岐）・`syncVoucherAccessLink()`の4箇所が生UTC値のまま返していたと特定。Codexへ委譲（**途中、Codexタスクが1時間以上ハングする事故が発生、`codex-companion.mjs status`で異常検知→`TaskStop`→`--fresh`で再投入し2分で完了。詳細: プロジェクトメモリ`feedback_codex_hang_diagnosis.md`**）、`test_sync.php` 56/0 PASS、commit `5549c56`、Beaver_betaへデプロイ済み（DB件数vouchers5892/customers828に変化なし確認）。書き込み系のためcurl実地テストはせずDodaikun側の次回同期で確認依頼中
- **⚠️派生の緊急回帰（完了・2026-09-08）**: 上記タイムゾーン修正直後、回帰ゲートが`POST /customers`の500エラーを検出。原因は`utcToJst()`が定義されている`sync_helpers.php`が`GET /customers/sync`分岐内でのみrequireされており、POST等の他ルートで未定義関数エラーになっていたため。`require_once`をファイル先頭の常時ロード群へ移動して修正（commit `2ea3f68`）、4テストファイル（test_sync/test_auth_gate/test_auth_gate_sync/test_auth_gate_unit）全PASS確認、Beaver_betaへ再デプロイ・DB件数変化なし確認、Dodaikunへ報告済み。さらに`test_push_responses_last_synced_at.php`の1ケースが旧仕様（生UTC一致）を期待したままだったのでJST変換一致へ更新（commit `dcdea26`、テストのみのためデプロイ不要）
