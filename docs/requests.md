...
## -8. AI（番頭）が得意先・案件・見積をBeaverへ直接登録できるようにしてほしい（2026-08-17、藤田晴樹よりPWA経由）

原文（意訳）: 「Beaverに案件登録できる？できないならAIが得意先や案件、見積もりを登録できるようにしてほしい」

### 現状確認（番頭・2026-08-17）
- Beaver APIには`api/routes/customers.php`・`api/routes/projects.php`・`api/routes/vouchers.php`が既に存在し、得意先・案件・伝票（見積含む）のCRUDエンドポイント自体はコード上ある
- ただし番頭（`C:\claude-workspace`セッション）側にはBeaver APIへの接続設定（ベースURL・認証情報）が無く、現状呼び出せない。`shared/manager/nippou_snapshot.py`の`snapshot_beaver()`も「Beaver API未疎通のためスキップ」と既知の状態でcron記録している
- 認証面: R-0109でBeaverにログイン基盤（auth-hub連携）を導入するが、これはブラウザ利用者向けのCookieセッション認証であり、番頭AIのようなサーバー間連携には別途サービスアカウント的な仕組みが必要（下記検討点参照）

### 要望内容
番頭（claude-workspaceのAIセッション）が会話中に「〇〇さんの見積もりをBeaverに登録して」等の指示を受けたら、Beaver APIを直接叩いて得意先・案件・見積伝票を作成できるようにしたい。

### 検討が必要な点
- 認証: AIからの書き込みをどう認可するか（サービスアカウント的なAPIキー発行か、R-0109のauth-hub連携完了を待つか）
- 権限範囲: AI経由の登録は「誰が登録したか」を記録できる必要がある（Beaver側で登録者識別の仕組みが必要、R-0109のスコープ外＝別要望）
- 番頭側: 接続設定（.env等）とAPIクライアントの実装が別途必要（claude-workspace側のタスク）
