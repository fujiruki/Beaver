...
## -6. R-0108: 納期等の日付を期限接近度で強調表示（要設計相談、2026-08-07発覚）
...
ユーザー自身が「議論して」と明示しているため、実装前に指揮役から具体案を提示し、確認を取ってから進める。

## -5. R-0107: 案件一覧のソートが、他の一覧画面を経由すると消える（バグ再発、2026-08-07発覚）

R-0080フィードバック（id=20、2026-08-07 07:21、藤田晴樹より）原文:

> 案件一覽のソートをしても、伝票一覽を開いてから案件一覽に戻るとソートが消えている。列幅は希望通り維持されている。これはバグです。ほかの一覽がある画面でも同じでしょう。修正して

R-0096 Phase2/2bで対応した「詳細画面から一覧へ戻る（useSmartBack）」パターンとは別経路のバグと推定（サイドバーの別リンクをクリックして他の一覧画面へ移動→サイドバーから元の一覧に戻る、という直接ナビゲーションで発生）。指揮役が調査してから仕様化する。

## -4. R-0106: 案件一覧の工数目安列を右揃えに（2026-08-07発覚）

R-0080フィードバック（id=19、2026-08-07 07:10、藤田晴樹より）原文:

> 工数目安は右揃え表示にしてほしい

## -3. R-0105: 左メニュー下部にビルド時刻を表示（2026-08-07、藤田晴樹より会話内で直接）

> 左メニューの下部に、ビルド時刻が表示されるようにできますか？そうすればいつのビルドかわかるので。そしてそれが1日以内と、1時間以内がわかるようにフォント色が変わるようにして

### 背景
R-0104のデプロイ直後、「まだ直っていない」という指摘を受けたが、実際はブラウザがリロードされていなかっただけだった（本人が確認・撤回）。この種の「今表示されているのがどのビルドか分からない」混乱を防ぐため、常時ビルド時刻を可視化したいという要望。

## -2. R-0104: 列幅ドラッグでまだ他列の幅が変わる（R-0103の追加報告、2026-08-06発覚）

R-0080フィードバック（id=17, 2026-08-06 23:47、id=18, 2026-08-06 23:58、藤田晴樹より）原文:

> id=17: この案件一覽の画面の、得意先の列の幅を半分にしてみますね、次の要望で送ります。
>
> id=18: はい、得意先列を縮めてみたのがこのスクショです。そしたら、得意先列は縮んだけど、開始日や納期の幅が大きくなっていませんか？私の希望は、得意先の幅が縮んだら、その右のステータス・開始日、納期、操作、工数目安の列は幅はそのままで左に寄って、工数目安列の右が余白になってほしい。もちろんそのときに得意先列より左にある案件コード、案件名の列の幅も変化しない。伝わってるかプロンプトで確認させてね

添付スクリーンショット2枚（`uploads/feedback/17/`, `uploads/feedback/18/`）で確認: 得意先列を縮めても総幅・他列の幅がほぼ変化していない＝R-0103の修正（`minWidth:100%`除去）だけでは不十分だったことを示す。

### 調査結果（指揮役、2026-08-07）
`frontend/src/pages/ProjectList.tsx`の列定義は`actions`列以外（案件コード・案件名・得意先・ステータス・開始日・納期・工数目安）に`width`未指定。`DataTable.tsx`の`colgroup`は`widths[col.key] ?? col.width`が両方undefinedの列には`<col>`へ幅指定を出さない。`table-layout: fixed`のブラウザ挙動により、明示幅のない列同士で残り幅が自動配分されるため、得意先列をドラッグで縮めると、明示幅のない他の列（開始日・納期等）が代わりに広がってしまう。R-0103はこの根本原因（明示幅のない列が伸縮を吸収する）を解消していなかった。

→ R-0104として起票。

## -1. R-0100: 請求書削除がcustomers.carry_forward_balanceを戻さないバグ（2026-08-06発覚、Codexレビューにて）

R-0098（Undo/Redo）設計のCodex独立レビューで発覚した既存バグ。`api/routes/invoices.php`のPOST（116〜119行目）は請求書作成時に`next_carry_forward`を受け取り`customers.carry_forward_balance`を更新するが、DELETE（127〜147行目）には対応する巻き戻し処理が無い。請求書を作成→削除しただけで、得意先の繰越残高が古いままズレる。

藤田晴樹さんの判断: 今すぐ別途修正する（R-0098の実装を待たない）。
