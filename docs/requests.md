...
## 34. 案件一覧の右側にステータス全種類のフィルタボタンを追加（2026-09-11、本番フィードバックid=49）

/readyoubouで本番フィードバックid=49を確認（`/projects`）。原文:

> 「案件一覽の右側にステータスの全種類のボタンをつくり、クリックでフィルタリングできるようにしてほしい」

## 33. 段取りボードのカレンダーが10/18より右へスクロールできない（2026-09-06、本番フィードバックid=48）

/readyoubouで本番フィードバックid=48を確認（`/dandori`）。原文:
