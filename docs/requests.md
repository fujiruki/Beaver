...
## 26. R-0135: 案件新規作成画面の得意先検索が名前しか対象になっていないように見える（2026-09-01発覚、要確認）

/readyoubouで本番フィードバックid=41を確認（案件新規作成画面）:

> 「得意先検索でキーワード検索の対象が得意先名だけになってる？住所や読みがなとか備考もキーワード検索対象にしてほしい。」

### 調査結果
`ProjectDetail.tsx`の得意先`ComboSelect`は`searchText`に`name, name_kana, tel, mobile, address1, address2, memo`を全て含めており（R-0083 Phase1、コミット`1278e8c`、2026-08-05本番デプロイ済み）、コード上はすでに名前以外でも検索できる状態になっている。

### 藤田晴樹さんの追加回答（2026-09-01、原文一部ローマ字）
> 見てみた。読みがなが「カドタグミ」という表記だった。こういう表記ゆれも対象にして。

実際に検索を試した結果の報告で、対象列が名前だけという話ではなく、`name_kana`が「カドタグミ」（カタカナ）のような**表記ゆれ**（読みの揺れ・カタカナ表記等）が検索でヒットしないケースを指している模様。`ComboSelect.tsx`の`normalize()`は全角英数・カタカナ→ひらがな相当への変換のみ行っており、長音・促音・濁点表記ゆれ等は正規化していない。

### 対応方針（要確認・具体例待ち）
「カドタグミ」でどの検索語を入力してヒットしなかったのか、具体的な再現手順（実際に入力した検索文字列）がまだ特定できていない。次回、藤田晴樹さんに再現手順を確認してから着手する。**今回のセッションでは実装しない**。
