...
## 12. R-071: 案件の保存ボタンが機能しない（バグ）

案件（プロジェクト）画面で保存ボタンを押しても保存されない（晴樹さん報告 2026-07-06）。

### 技術メモ
- R-067（得意先詳細の保存ボタン、2026-07-02 修正済み）と同一パターンの疑い: `<form>` に `noValidate` がなく、不正値のあるフィールド（`<input type="email">` 等）のネイティブHTML5検証が submit を黙ってブロックする
- まず ProjectDetail 系フォームの実コードで真因を確認してから修正すること（推定のまま直さない）

---

## 13. R-027b: 本番の日次バックアップが停止している疑い（バグ・要調査）

2026-07-06 のデプロイ列車（migration 018/019/020 本番適用）作業中に発覚。

### 症状
- R-027（定時バックアップ機能、2026-06-07 完了記録あり）で導入したはずの `backup.sh` が本番 Beaver ディレクトリに存在しない（`find` でも発見できず）
- `api/backups/` 配下の最新バックアップファイルが 2026-06-16 時点で止まっている（以降のバックアップが作られていない）
- crontab（`0 3 * * * .../Beaver/backup.sh`）が実際に機能しているか未確認。スクリプト不在なら当然失敗し続けているはず

### 優先度
【高】。バックアップが長期間取れていない状態は障害時のデータ復旧リスクに直結するため優先度高。

### 調査・復旧の方向性
- 本番SSHで `backup.sh` の実在・crontab登録状況を確認
- スクリプトが消失した経緯（別デプロイでの誤削除等）を可能な範囲で特定
- スクリプト再設置＋crontab再登録、直近バックアップの動作確認まで実施
