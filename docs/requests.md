...
## 29. R-0137: 上部の保存ボタンが隠れる（2026-09-01発覚、要確認・再現手順待ち）

/readyoubouで本番フィードバックid=44を確認（案件一覧画面、投稿時点のpage_path）:

> 「上部保存ボタンが隠れてる。なおして隠れるように重なったものがあるのはよくないバグだよね」

### 調査結果（未確定）
文面はR-0131（2026-08-31本番デプロイ済み・スマホでヘッダーがページ上部のボタン・行を隠す不具合）と酷似しているが、指揮役が現在のコードを確認したところ`AppLayout.tsx`の`<main className="pt-14 md:pt-6">`（R-0131の修正）は現在も維持されており、リグレッションは見当たらない。以下のいずれかを疑っているが、実機の再現手順が無く特定できていない:

1. PWA/ブラウザキャッシュにより、R-0131修正前の古いJSバンドルがまだ表示されている（R-0132・R-0135と同系統のキャッシュ問題の疑い）
2. R-0131の`<main>`修正がカバーしていない別の画面固有の固定要素（例: モーダル・ドロワー等）で、「保存」ボタンが別の要素に隠れている
3. iOS Safariでホーム画面に追加した際の表示（`viewport-fit=cover`・`safe-area-inset`未対応、`apple-mobile-web-app-capable`meta・`manifest.json`も未整備で真の意味のstandalone起動になっていない）に起因する、Chrome DevTools Protocolのモバイルエミュレーションでは再現しない実機固有の見え方

### 対応方針（要確認・具体例待ち）
藤田晴樹さんに以下を確認してから着手する:
- 発生した画面（案件一覧か、案件編集・伝票編集などの保存ボタンがある画面か）
- ブラウザで直接開いた場合とPWA（ホーム画面アイコン）から開いた場合、どちらで発生したか
- 可能であればスクリーンショット
