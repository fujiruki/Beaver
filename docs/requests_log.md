...
| 2026-08-05 | R-0080: 改善要望を伝えるフィードバックフォーム（画像複数添付） | 仕様化済み | Beaver内どの画面からも開ける「改善要望を送る」ボタン→本文＋画像最大5枚を送信、`feedback`/`feedback_images`テーブルに保存。開発側（Claude）が`GET /admin/feedback`（`X-Admin-Token`認証）で本番データを直接読みに行く運用。詳細仕様: `docs/spec/R-0080_feedback_form.md` |
